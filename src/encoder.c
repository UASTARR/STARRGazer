...
int main(int argc, char *argv[]) {
    GstElement *source, *convert, *encoder, *parser, *muxer, *sink;
    GstBus *bus;
    GstMessage *msg;
    ...
    if (!pipeline || !source || !convert || !encoder || !muxer || !parser || !sink) {
    ...
    g_object_set(sink, "location", "outputh264.mp4", NULL);

    /* Build the pipeline */
    gst_bin_add_many(GST_BIN(pipeline), source, convert, encoder, parser, muxer, sink, NULL);
